...
var monthNum_small = todaysMonth;
var yearNum_small = todaysYear;
...
function createCalendar(layout, firstDay, numbDays, monthNum, yearNum) { 
	...
	if (layout == 'full') {
		...
		calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedate(\'prevyr\', \'small\')\">«<\/span><\/td>';
		calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedate(\'prevmo\', \'small\')\">«<\/span><\/td>';
		calendarString += '<td class=\"calendar-title\" colspan=\"3\"><span>' + wordMonth[monthNum - 1] + '&nbsp;&nbsp;' + yearNum + '<\/span><\/td>';
		calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedate(\'nextmo\', \'small\')\">»<\/span><\/td>';
		calendarString += '<td class=\"calendar-btn\"><span onClick=\"changedate(\'nextyr\', \'small\')\">»<\/span><\/td>';
	}
	calendarString += '<\/tr>';
					...
					if ((todaysDate == daycounter) && (todaysMonth == monthNum)) {
						...
						if (checkEvents(daycounter, monthNum, yearNum)) {
							if (layout == 'full') {
								...
								calendarString += '<div class=\"calendar-day-event\" onmouseover=\"showTooltip(0, \'small\', ' + daycounter + ', ' + monthNum + ', ' + yearNum + ', this)\" onmouseout=\"clearTooltip(\'small\', this)\" onclick=\"showEventDetail(0, \'small\', '  + daycounter + ', ' + monthNum + ', ' + yearNum + ')\">';
							}
								...
								// Tooltip
								calendarString += '<div class=\"event-tooltip\"><\/div>';
							calendarString += '<\/div>';
						} else {
							calendarString += daycounter_s;
						}
	...
	if (layout == 'full') {
		jQuery('.calendar-full').html(calendarString);
	} else {
		jQuery('.calendar-small').html(calendarString);
	}
	thisDate = 1;
}
...
function getEvents(day, month, year) {
	...
	for (var i = 0; i < tiva_events.length; i++) {
		...
		if ((start_date.getTime() <= date_check.getTime()) && (date_check.getTime() <= end_date.getTime())) {
			...
			var event = {id:tiva_events[i].id, short_name:tiva_events[i].short_name, name:tiva_events[i].name, first_day:first_day, last_day:last_day, color:tiva_events[i].color};
...
function showTooltip(id, layout, day, month, year, el) {
	if (layout == 'full') {
		...
		jQuery(el).parent().find('.event-tooltip').css('bottom', bottom + 'px');
		
		jQuery(el).parent().find('.event-tooltip').html(	'<div class="event-tooltip-item">'
																+ event_time
																+ '<div class="event-name">' + tiva_events[id].short_name + '</div>'
																+ '<div class="event-image">' + event_image + '</div>'
																+ '<div class="event-intro">' + getShortText(tiva_events[id].description, 10) + '</div>'
																+ '</div>'
															);
		jQuery(el).parent().find('.event-tooltip').css('opacity', '1');
		jQuery(el).parent().find('.event-tooltip').css('-webkit-transform', 'translate3d(0,0,0) rotate3d(0,0,0,0)');
		jQuery(el).parent().find('.event-tooltip').css('transform', 'translate3d(0,0,0) rotate3d(0,0,0,0)');
	} else {
		jQuery(el).find('.event-tooltip').html('');
		...
		for (var i = 0; i < events.length; i++) {
			if (typeof events[i] != "undefined") {
				...
				jQuery(el).find('.event-tooltip').append(	'<div class="event-tooltip-item">'
																+ event_time
																+ '<div class="event-name">' + tiva_events[events[i].id].short_name + '</div>'
																+ '<div class="event-image">' + event_image + '</div>'
		...
		jQuery(el).find('.event-tooltip').css('opacity', '1');
		jQuery(el).find('.event-tooltip').css('-webkit-transform', 'translate3d(0,0,0) rotate3d(0,0,0,0)');
		jQuery(el).find('.event-tooltip').css('transform', 'translate3d(0,0,0) rotate3d(0,0,0,0)');
	}
}
...
function clearTooltip(layout, el) {
	if (layout == 'full') {
		jQuery(el).parent().find('.event-tooltip').css('opacity', '0');
		jQuery(el).parent().find('.event-tooltip').css('-webkit-transform', 'translate3d(0,-10px,0)');
		jQuery(el).parent().find('.event-tooltip').css('transform', 'translate3d(0,-10px,0)');
	} else {
		jQuery(el).find('.event-tooltip').css('opacity', '0');
		jQuery(el).find('.event-tooltip').css('-webkit-transform', 'translate3d(0,-10px,0)');
		jQuery(el).find('.event-tooltip').css('transform', 'translate3d(0,-10px,0)');
	}
}
...
function showEventList(layout, max_events) {
	...
	if (layout == 'full') {
		...
		for (var i = 0; i < tiva_list_events.length; i++) {
			...
			jQuery('.event-list-full').append(	'<div class="event-item">'
		...
		for (var i = 0; i < tiva_list_events.length; i++) {
			...
			jQuery('.event-list-small').append(	'<div class="event-item">'
															+ '<div class="event-image link" onclick="showEventDetail(' + tiva_list_events[i].id + ', \'small\', 0, 0, 0)">' + event_image + '</div>'
															+ '<div class="event-name link" onclick="showEventDetail(' + tiva_list_events[i].id + ', \'small\', 0, 0, 0)">' + tiva_list_events[i].name + '</div>'
															+ '<div class="event-date"><i class="fa fa-calendar-o"></i>' + event_day + ', ' + event_date + event_end_time + '</div>'
															+ '<div class="event-time">' + event_time + '</div>'
...
function showEventDetail(id, layout, day, month, year) {
	jQuery('.events-calendar.' + layout + ' .back-calendar').show();
	jQuery('.events-calendar.' + layout + ' .calendar').hide();
	jQuery('.events-calendar.' + layout + ' .event-list').hide();
	jQuery('.events-calendar.' + layout + ' .event-detail').fadeIn(1500);
	
	jQuery('.events-calendar.' + layout + ' .list-view').removeClass('active');
	jQuery('.events-calendar.' + layout + ' .calendar-view').removeClass('active');
	...
	if (layout == 'full') {
		...
		jQuery('.event-detail-full').html(	'<div class="event-item">'
													+ '<div class="event-image">' + event_image + '</div>'
		...
		jQuery('.event-detail-small').html('');
		if (day && month && year) {
			var events = getEvents(day, month, year);
		} else {
			var events = [{id:id}];
		...
		for (var i = 0; i < events.length; i++) {
			if (typeof events[i] != "undefined") {
				...
				jQuery('.event-detail-small').append( 	'<div class="event-item">'
																	+ '<div class="event-image">' + event_image + '</div>'
																	+ '<div class="event-name">' + tiva_events[events[i].id].short_name + '</div>'
																	+ '<div class="event-date"><i class="fa fa-calendar-o"></i>' + event_day + ', ' + event_date + event_end_time + '</div>'
																	+ '<div class="event-time">' + event_time + '</div>'
...
jQuery(document).ready(function(){
	// Init calendar full
	if (jQuery('.events-calendar.full').length) {
		jQuery('.events-calendar.full').html(	'<div class="events-calendar-bar">'
													...
													+ '<div class="events-calendar-wrap">'
														+ '<div class="calendar-full calendar"></div>'
														+ '<div class="event-list-full event-list"></div>'
														+ '<div class="event-detail-full event-detail"></div>'
													+ '</div>'
												);
	}
	
	// Init calendar small
	if (jQuery('.events-calendar.small').length) {
		jQuery('.events-calendar.small').html(	'<div class="events-calendar-bar">'
															+ '<span class="bar-btn calendar-view active"><i class="fa fa-calendar-o"></i>' + calendar_view + '</span>'
															+ '<span class="bar-btn list-view"><i class="fa fa-list"></i>' + list_view + '</span>'
															+ '<span class="bar-btn back-calendar pull-right active"><i class="fa fa-caret-left"></i>' + back_small + '</span>'
														+ '</div>'
														+ '<div class="cleardiv"></div>'
														+ '<div class="events-calendar-wrap">'
															+ '<div class="calendar-small calendar"></div>'
															+ '<div class="event-list-small event-list"></div>'
															+ '<div class="event-detail-small event-detail"></div>'
														+ '</div>'
													);
	}
	
	// Show - Hide view
	jQuery('.events-calendar .back-calendar').hide();
	jQuery('.event-list').hide();
	jQuery('.event-detail').hide();
	
	jQuery('.events-calendar').each(function(index) {
		// Hide switch button
		...
		if (switch_button == 'hide') {
			...
			jQuery(this).find('.event-detail').css('padding-top', '60px');
		}
	});
	
	// Set wordDay
	date_start = (typeof jQuery('.events-calendar').attr('data-start') != "undefined") ? jQuery('.events-calendar').attr('data-start') : 'sunday';
	if (date_start == 'sunday') {
	...
	jQuery.ajax({
		...
		data: '',
		beforeSend : function(){
			jQuery('.calendar').html('<div class="loading"><img src="images/temp/loading.gif" /></div>');
		},
	...
	jQuery('.events-calendar .calendar-view').click(function(){
		jQuery(this).parents('.events-calendar').find('.back-calendar').hide();
		jQuery(this).parents('.events-calendar').find('.event-list').hide();
		jQuery(this).parents('.events-calendar').find('.event-detail').hide();
		jQuery(this).parents('.events-calendar').find('.calendar').fadeIn(1500);
		
		jQuery(this).parents('.events-calendar').find('.list-view').removeClass('active');
		jQuery(this).parents('.events-calendar').find('.calendar-view').addClass('active');
	});

	// Click - List view btn
	jQuery('.events-calendar .list-view').click(function(){
		jQuery(this).parents('.events-calendar').find('.back-calendar').hide();
		jQuery(this).parents('.events-calendar').find('.calendar').hide();
		jQuery(this).parents('.events-calendar').find('.event-detail').hide();
		jQuery(this).parents('.events-calendar').find('.event-list').fadeIn(1500);
		
		jQuery(this).parents('.events-calendar').find('.calendar-view').removeClass('active');
		jQuery(this).parents('.events-calendar').find('.list-view').addClass('active');

		var layout = jQuery(this).parents('.events-calendar').attr('class') ? jQuery(this).parents('.events-calendar').attr('class') : 'full';
		var max_events = jQuery(this).parents('.events-calendar').attr('data-events') ? jQuery(this).parents('.events-calendar').attr('data-events') : 1000;
		if (layout.indexOf('full') != -1){
			showEventList('full', max_events);
		} else {
			showEventList('small', max_events);
		}
	});

	// Click - Back calendar btn
	jQuery('.events-calendar .back-calendar').click(function(){
		jQuery(this).parents('.events-calendar').find('.back-calendar').hide();
		jQuery(this).parents('.events-calendar').find('.event-detail').hide();
		
		var initial_view = (typeof jQuery(this).parents('.events-calendar').attr('data-view') != "undefined") ? jQuery(this).parents('.events-calendar').attr('data-view') : 'calendar';
